...
git clone git@github.com:Powerloom/rate-limiter.git
cd rate-limiter
git checkout $RATE_LIMITER_BRANCH
cd ..

git clone git@github.com:Powerloom/snapshotter-periphery-blockfetcher.git
cd snapshotter-periphery-blockfetcher
git checkout $SNAPSHOTTER_PERIPHERY_BLOCKFETCHER_BRANCH
cd ..

git clone git@github.com:Powerloom/snapshotter-periphery-txprocessor.git
cd snapshotter-periphery-txprocessor
git checkout $SNAPSHOTTER_PERIPHERY_TXPROCESSOR_BRANCH
cd ..

git clone git@github.com:Powerloom/snapshotter-periphery-epochsyncer.git
